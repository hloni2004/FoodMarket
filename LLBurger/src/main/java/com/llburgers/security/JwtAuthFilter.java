...
/**
 * Intercepts every request, extracts the JWT access token from the
 * {@code Authorization: Bearer <token>} header, validates it, and populates
 * the Spring Security context so downstream filters and controllers can
        ...
        try {
            if (jwtService.isTokenValid(token) && jwtService.isAccessToken(token)) {
                ...
                if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                    ...
                    log.debug("[JWT-AUTH] Authenticated user: {}", email);
                }
            }
        } catch (Exception e) {
